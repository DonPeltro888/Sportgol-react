...
const StickySearch = () => {
  ...
  return (
    <div className="sticky top-[64px] z-[1000] bg-gray-900/95 backdrop-blur-md border-b border-gray-800 py-3 px-4">
      <div ref={searchRef} className="container mx-auto max-w-3xl relative">
        {/* Search Input */}
        <div className="relative flex items-center bg-gray-800 border border-gray-700 rounded-xl overflow-hidden focus-within:border-blue-500 transition-colors">
          ...
          <input
            ...
            className="flex-1 pl-12 pr-10 py-3 bg-transparent text-white placeholder-gray-500 outline-none text-sm"
        ...
        {/* Dropdown Results */}
        {showDropdown && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-y-auto max-h-[350px] z-[1001]">
            {loading ? (
              <div className="flex items-center justify-center py-6">
                <Loader2 className="w-5 h-5 text-blue-400 animate-spin" />
                <span className="ml-2 text-gray-400 text-sm">{t('loadingEvents')}</span>
              </div>
            ) : totalResults > 0 ? (
              <div>
                {/* Header */}
                <div className="px-3 py-2 bg-gray-800/50 border-b border-gray-700">
                  <span className="text-xs text-gray-400">{totalResults} {t('resultsFound')}</span>
                </div>

                {/* Teams */}
                {teamResults.length > 0 && (
                  <div>
                    <div className="px-3 py-1.5 bg-blue-600/20 border-b border-gray-700 flex items-center gap-2">
                      <Users className="w-3 h-3 text-blue-400" />
                      <span className="text-xs font-semibold text-blue-400">{t('teams')}</span>
                    </div>
                    <div className="p-1.5">
                      {teamResults.map((team) => {
                        ...
                        return (
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-2 py-1.5 hover:bg-blue-600/20 rounded-lg transition-colors text-left"
                          >
                            {logo ? (
                ...
                {eventResults.length > 0 && (
                  <div>
                    <div className="px-3 py-1.5 bg-purple-600/20 border-b border-gray-700 flex items-center gap-2">
                      <Trophy className="w-3 h-3 text-purple-400" />
                      <span className="text-xs font-semibold text-purple-400">{t('eventsLabel')}</span>
                    </div>
                    {eventResults.map((event) => (
                      ...
                      >
                        <div className="flex-shrink-0 w-10 text-center">
                          <div className="text-xs font-bold text-purple-400">{formatDate(event.date)}</div>
                        </div>
